...
<div class="form-group row">
    <label for="school" class="col-md-2 col-form-label text-md-right">School</label>
        <div class="col-md-8">
{{--     country --}}
    <select name="school" id="school" class="selectpicker form-control input-lg dynamic @error('gedsemid') is-invalid @enderror @error('gedtriid') is-invalid @enderror" data-dependent="program" data-live-search="true" title="Select School" >
                @foreach($members as $srebmem)
            <option value="{{$srebmem->id}}" >{{$srebmem->school}}</option>
...
<div class="form-group row">
    <label for="agp" class="col-md-2 col-form-label text-md-right">Accredited Graduate Programs</label>
        <div class="col-md-8">
{{--     country --}}
    <select name="agp[]" id="agp" class="selectpicker form-control input-lg @error('gedsemagp') is-invalid @enderror @error('gedtriagp') is-invalid @enderror" multiple data-live-search="true" title="Check All Accredited Graduate Programs">
                @foreach($agp as $pca)
...
<div class="form-group row">
    <label for="program" class="col-md-2 col-form-label text-md-right">Programs</label>
        <div class="col-md-8">
{{--     state --}}
    <select name="program[]" id="program" class="selectpicker form-control input-lg @error('gedsemprogram') is-invalid @enderror @error('gedtriprogram') is-invalid @enderror" multiple data-dependent="city" title="Check All Programs">
    </select>
                @error('gedsemprogram')
                <span class="invalid-feedback" role="alert">
                <strong>{{ $message }}</strong>
